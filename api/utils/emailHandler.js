...
const sendPostBanEmail = async ({
  ...
  await transporter.sendMail({
    from: "Day Keeper <daykeepeer655@gmail.com>",
    to: email,
    subject: `Seu post do dia ${date} foi banido do DayKeeper`,
    text: ` Após uma analise da sua conta por nossos admininstradores, decidimos banir seu post
            do dia ${date}, de id "${id}", pertencente a sua conta de nome ${username}, do DayKeeper.
...
const sendPostUnbanEmail = async ({
  ...
  await transporter.sendMail({
    from: "Day Keeper <daykeepeer655@gmail.com>",
    to: email,
    subject: `Seu post do dia ${date} foi DESBANIDO do DayKeeper ;)`,
    text: `Post do dia ${bannedPostdate} com o id "${id}", da conta ${username}
...
const sendPostDeletionEmail = async ({
  ...
  await transporter.sendMail({
    from: "Day Keeper <daykeepeer655@gmail.com>",
    to: email,
    subject: `Seu post do dia ${date} foi excluido permanentemente do DayKeeper`,
    text: ` Por falta de resposta ou por confirmação de admininstradores, decidimos excluir permanentemente
            seu post do dia ${date} com o id "${id}", pertencente a sua conta de nome ${username}, no DayKeeperr.
